...
import { DEMO_GAMIFICATION_DATA } from './demoData'
...
// Create demo badges by merging initial badges with demo unlocked badges
const getDemoBadges = (): Badge[] => {
  const demoBadges = [...INITIAL_BADGES]
  
  // Update badges that should be unlocked in demo
  DEMO_GAMIFICATION_DATA.badges.forEach(demoBadge => {
    const index = demoBadges.findIndex(b => b.id === demoBadge.id)
    if (index !== -1) {
      demoBadges[index] = { ...demoBadges[index], ...demoBadge }
    }
  })
...
export const useGamification = create<GamificationState>()(
  persist(
    (set) => ({
      stats: {
        ...DEMO_GAMIFICATION_DATA.stats,
        badges: getDemoBadges(),
        dailyQuests: DEMO_GAMIFICATION_DATA.dailyQuests
      },
