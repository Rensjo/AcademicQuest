...
// Helper function to convert demo attendance data to store format
const convertDemoAttendanceToRecords = (): AttendanceRecord[] => {
  const recordsMap = new Map<string, ClassAttendance[]>()
  
  // Group attendance by date
  DEMO_ATTENDANCE.forEach(entry => {
    if (!recordsMap.has(entry.date)) {
      recordsMap.set(entry.date, [])
    }
    
    const classAttendance: ClassAttendance = {
      date: entry.date,
      slotId: entry.courseId,
      courseCode: entry.courseId.split('-')[0].toUpperCase(),
      courseName: entry.courseId.split('-')[0].toUpperCase(),
      attended: entry.status === 'present',
      marked: true,
      time: '09:00' // Default time
    }
    
    recordsMap.get(entry.date)!.push(classAttendance)
  })
  
  // Convert to AttendanceRecord format
  const records: AttendanceRecord[] = []
  recordsMap.forEach((classes, date) => {
    const attendedCount = classes.filter(c => c.attended).length
    const attendanceRate = Math.round((attendedCount / classes.length) * 100)
...
export const useAttendance = create<AttendanceState>()(
  persist(
    (set, get) => ({
      records: convertDemoAttendanceToRecords(),
      ...
      getLast365DaysData: () => {
        ...
        // For demo purposes, use the last date in demo data as "now"
        const demoEndDate = new Date('2025-09-05') // Last date in demo data
        const oneYearAgo = new Date(demoEndDate)
        oneYearAgo.setDate(oneYearAgo.getDate() - 364) // 365 days total

        // Generate array of last 365 days from demo end date
        const last365Days = []
        for (let i = 364; i >= 0; i--) {
          const date = new Date(demoEndDate)
